        ...
        <div className="w-full space-y-3 pt-4 text-left">
          ...
          {user.lastActiveAt && (
            <div className="flex items-start text-muted-foreground">
              <Clock className="mr-3 mt-0.5 h-4 w-4 shrink-0 text-muted-foreground/70" />
              <span className="text-sm">
                {t("last_active")}{" "}
                {format(new Date(user.lastActiveAt), "PP", {
                  locale: dateLocale,
                })}
              </span>
            </div>
          ...
          {user.isPremium && (
            <div className="mt-4 rounded-lg border border-purple-500/20 bg-gradient-to-r from-purple-500/10 to-blue-500/10 p-3">
              ...
              {user.premiumStartedAt && (
                <div className="break-words text-xs text-muted-foreground">
                  {t("premium_since")}{" "}
                  {format(new Date(user.premiumStartedAt), "PP", {
                    locale: dateLocale,
                  })}
                </div>
              )}
              {user.premiumExpiresAt && (
                <div className="break-words text-xs text-muted-foreground">
                  {t("premium_expires")}{" "}
                  {format(new Date(user.premiumExpiresAt), "PP", {
                    locale: dateLocale,
                  })}
                </div>
              )}
