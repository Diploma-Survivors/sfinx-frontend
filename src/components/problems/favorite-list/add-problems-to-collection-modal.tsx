...
export function AddProblemsToCollectionModal({
  ...
  const fetchProblems = async (query: string, pageNum: number) => {
    ...
    try {
      ...
      error(t("failed_to_load_problems"));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleAddProblems = async () => {
    ...
    try {
      ...
      error(t("failed_to_add_problems"));
    } finally {
      setIsAdding(false);
    }
  };
          ...
          >
            {problems.length > 0
              ? problems.map((problem) => {
                  const isAlreadyInList = currentProblemIds.includes(
                    problem.id,
                  );
              ...
              : !isLoading && (
                  <div className="p-4 text-center text-sm text-muted-foreground">
                    {searchQuery ? t("no_results") : t("start_typing")}
                  </div>
                )}
