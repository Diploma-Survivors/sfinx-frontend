  ...
  const handleToggleSave = async () => {
    try {
      ...
      if (isSaved) {
        ...
        setIsSaved(false);
        toastService.success(t("list_unsaved"));
      } else {
        await favoriteListService.save(list.id);
        setIsSaved(true);
        toastService.success(t("list_saved"));
      }
      onListUpdated?.();
    } catch (error) {
      console.error("Failed to toggle save list", error);
      toastService.error(t("error_saving"));
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const renderIcon = (icon: string) => {
    ...
    return (
      <div
        ...
        title={isOwner ? t("change_icon") : undefined}
      >
        {content}
        {isOwner && (
          <>
            ...
            <div>
              ...
              <div className="mt-2 flex items-center gap-2 text-sm text-muted-foreground">
                <span className="font-medium text-foreground">
                  {list.user?.username || t("user_fallback")}
                </span>
                <span>•</span>
                <span>
                  {totalProblems} {t("questions")}
                </span>
                <span>•</span>
                <span>{list.isPublic ? t("public") : t("private")}</span>
              </div>
            </div>
                    ...
                    return (
                      <div key={difficulty} className="space-y-1">
                        ...
                        <Progress
                          value={percentage}
                          className="h-1.5"
                          indicatorClassName={stats.color.replace(
                            "text-",
                            "bg-",
                          )}
                        />
