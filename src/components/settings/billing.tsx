  ...
  return (
    <div className="space-y-6">
      <Card className="border-border/40 shadow-sm">
        <CardHeader>
          <CardTitle>
            {t("subscription_plan", { defaultValue: "Subscription Plan" })}
          </CardTitle>
          <CardDescription>
            {t("manage_subscription", {
              defaultValue: "Manage your subscription details.",
            })}
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <div className="text-lg font-semibold text-foreground">
              {currentPlan
                ? currentPlan.name
                : t("free_plan", { defaultValue: "Free Plan" })}
            </div>
            {currentPlan && currentPlan.status === "ACTIVE" ? (
              <div className="mt-1 space-y-1">
                <p className="text-sm text-green-600 font-medium">
                  {t("premium_active_until", { defaultValue: "Active until" })}{" "}
                  {formatDate(currentPlan.expiresAt)}
                </p>
                <p className="text-xs text-muted-foreground">
                  {currentPlan.daysRemaining} {t("days_remaining")}
                </p>
              </div>
            ) : (
              <p className="text-sm text-muted-foreground mt-1">
                {t("upgrade_to_unlock", {
                  defaultValue: "Upgrade to unlock premium features.",
                })}
              </p>
            )}
          </div>
          <Link href="/pricing">
            <Button className="bg-accent text-accent-foreground hover:bg-accent/90 shadow-sm">
              {currentPlan
                ? t("extend_plan", { defaultValue: "Extend Plan" })
      ...
      <Card className="border-border/40 shadow-sm">
        <CardHeader>
          <CardTitle>
            {t("billing_info", { defaultValue: "Billing Information" })}
          </CardTitle>
          <CardDescription>
            {t("billing_info_desc", {
              defaultValue: "View your billing status.",
            })}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {transactions.filter((t) => t.status === PaymentStatus.SUCCESS)
            .length > 0 ? (
            ...
            <div className="text-center py-8 text-muted-foreground bg-muted/20 rounded-lg border border-dashed border-border">
              <p>
                {t("no_billing_history", {
                  defaultValue: "No billing history available.",
                })}
              </p>
            </div>
          )}
