...
import type { StudyPlanDetailResponseDto, StudyPlanProgressResponseDto, EnrollmentStatus, StudyPlanCardResponseDto } from '@/types/study-plans';
...
export default function useStudyPlan(slug: string): UseStudyPlanReturn {
  ...
  const [progress, setProgress] = useState<StudyPlanProgressResponseDto | null>(null);
  ...
  const fetchDetail = useCallback(async () => {
    ...
    try {
      ...
      if (res.data?.data?.isEnrolled) {
        const progRes = await StudyPlansService.getStudyPlanProgress(res.data.data.id, lang);
      ...
      if (res.data?.data?.id) {
        try {
          ...
          const similarRes = await StudyPlansService.getSimilarPlans(res.data.data.id, lang);
